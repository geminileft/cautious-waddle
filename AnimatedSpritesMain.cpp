...
AnimatedSpritesMain::AnimatedSpritesMain(const std::shared_ptr<DX::DeviceResources>& deviceResources) :
	m_deviceResources(deviceResources), m_backgroundSpeed(500)
{
	...
	InputSourceManager* inputManager = InputSourceManager::InitPlatformManager(m_deviceResources->GetWindow());
	m_inputSystem = std::unique_ptr<InputSystem>(new InputSystem(inputManager));
...
void AnimatedSpritesMain::Update() 
{
	...
	m_timer.Tick([&]()
	{
		...
		m_inputSystem->Update(m_timer);
		auto buttons = m_inputSystem->GetButtons();
		
		// Check if menu is pressed
		if ((buttons & InputButtons::Option1) == InputButtons::Option1)
		{
			std::string message = "Are you sure you want to quit?";
		}
		else if ((buttons & InputButtons::DPadRight) == InputButtons::DPadRight)
		{
			m_shipPos.x += 1.0;
		}
		else if ((buttons & InputButtons::DPadLeft) == InputButtons::DPadLeft)
		{
			m_shipPos.x -= 1.0;
		}
		else if ((buttons & InputButtons::DPadDown) == InputButtons::DPadDown)
		{
			m_backgroundSpeed -= 50;
		}
		else if ((buttons & InputButtons::DPadUp) == InputButtons::DPadUp)
		{
			m_backgroundSpeed += 50;
		}
		...
		m_stars->Update(m_timer.GetElapsedSeconds() * m_backgroundSpeed);
	});
}
